...
function addCartClicked(event) {
    ...
    var title = shopProduct.querySelector(".card-title").textContent;
    var price = shopProduct.querySelector(".card-text").textContent;
    var productImg = shopProduct.querySelector(".card-img-top").src;
...
function addProductToCart(title, price, productImg) {
    ...
    var cartItemsStorage = localStorage.getItem("cartItems");
    var cartItems = cartItemsStorage ? JSON.parse(cartItemsStorage) : [];
...
function updatetotal() {
    ...
    for (var i = 0; i < cartBoxes.length; i++) {
        ...
        var quantityElement = cartBox.querySelector(".cart-quantity").value;
        var price = parseFloat(priceElement.replace(".", "").replace("đ", ""));
...
// Cập nhật Local Storage khi thay đổi giỏ hàng
function updateLocalStorage() {
    ...
    for (var i = 0; i < cartBoxes.length; i++) {
        var cartBox = cartBoxes[i];
        var title = cartBox.querySelector(".cart-product-title").textContent;
        var price = cartBox.querySelector(".cart-price").textContent;
